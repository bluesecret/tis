  ...
  >
    ...
    <template v-for="tableColumn in tableColumnList" :key="tableColumn.id">
      ...
      >
        <template #default="{ row }">
          <upload-file-list
            :file-list="parseTableUploadData(tableColumn, row)"
            type="card"
...
const onPrint = (operation: ANY_OBJECT, row: ANY_OBJECT) => {
  ...
  post<string>(API_CONTEXT + '/flow/flowOperation/printWorkOrder/', params)
    .then(res => {
      let downloadUrl = res.data;
      let fileName = form().formName + (operation.printTemplateType === 1 ? '.docx' : '.xlsx');
      download(downloadUrl, {}, fileName, 'post').catch(e => {
        console.log(e);
        ElMessage.error(e);
      });
    })

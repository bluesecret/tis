...
watch(
  () => layoutStore.currentTagId,
  (newVal, oldVal) => {
    if (!layoutStore.supportTags) return;
    let currentTag = layoutStore.tagList.find(item => item.id === newVal);
    if (currentTag != null) {
      setTimeout(() => {
        router.push({ name: currentTag.routerName, query: currentTag.query });
      });
    }
  },
);

const oauthBindInfo = ref([]);
const loadOAuthBindInfo = () => {
  OAuthController.getBindInfo()
    .then(res => {
      oauthBindInfo.value = res.data;
    })
    .catch(e => {
      console.error(e);
    });
...
const showOAuthItem = source => {
  for (let index = 0; index < oauthBindInfo.value.length; index++) {
    const element = oauthBindInfo.value[index];

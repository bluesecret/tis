...
const emitChange = () => {
  ...
  if (pps.multiple) {
    ...
    tempValue = selectedItems.value.map(item => {
      ...
      dictData.push({
        id: item[pps.props.value],
        name: item[pps.props.label],
      });
      return item;
    });
    emit(
    ...
    let dictData = {
      id: tempValue[pps.props.value],
      name: tempValue[pps.props.label],
    };
    emit('update:modelValue', tempValue[pps.props.value]);
    emit('change', tempValue[pps.props.value], selectedItems.value, dictData);
  }
};

            ...
            >
              ...
              >
                <el-row :gutter="16">
                  ...
                  <el-col :span="12" v-if="formFlowEntryData.flowType === FlowEntryType.NORMAL">
                    <el-form-item label="流程复活">
                      <el-switch v-model="formFlowEntryData.supportRevive" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="12" v-if="formFlowEntryData.flowType === FlowEntryType.NORMAL">
                    <el-form-item label="有效天数">
                      <el-input-number
                        v-model="formFlowEntryData.keptReviveDays"
                        :disabled="!formFlowEntryData.supportRevive"
                        placeholder="不输入代表永久有效"
                  ...
                  <el-col
                    key="autoParamList"
                    :span="24"
                    v-if="formFlowEntryData.flowType === FlowEntryType.AUTO_TASK"
                  >
                    <el-form-item label="输入参数">
                      <el-select
                        v-model="autoParamList"
                        multiple
                        filterable
                        allow-create
...
const formFlowEntryData = ref<ANY_OBJECT>({
  ...
  autoParamJson: undefined,
});
const autoParamList = ref<ANY_OBJECT[]>([]);
...
const saveFlowEntryInfo = () => {
  ...
  let params = {
    flowEntryDto: {
      ...
      autoParamJson: JSON.stringify(autoParamList.value.map(item => ({ variableName: item }))),
    },
  };
  console.log(
...
const changeDiagram = () => {
  formFlowEntryData.value.diagramType =
    formFlowEntryData.value.diagramType === DiagramType.ORDINARY
      ? DiagramType.DINGDING
      : DiagramType.ORDINARY;
  designer.value.processDesigner.changeDiagram();
};
const onSave = () => {
...
const initFlowEntryInfo = () => {
  ...
  FlowEntryController.view({
    ...
    .then(res => {
      ...
      autoParamList.value = JSON.parse(formFlowEntryData.value.autoParamJson || '[]');
      autoParamList.value = autoParamList.value.map(item => item.variableName);
      if (formFlowEntryData.value.bindFormType === SysFlowEntryBindFormType.ONLINE_FORM) {

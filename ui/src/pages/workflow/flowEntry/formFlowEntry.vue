      ...
      <el-row>
        <el-col :span="24">
          <el-table :data="currentAutoTask.autoParamJson" height="300px">
            ...
            <el-table-column prop="variableValue" label="参数值">
              <template v-slot="scope">
                <el-input size="default" v-model="scope.row.variableValue" clearable />
              </template>
            </el-table-column>
          </el-table>
        </el-col>
        <el-col :span="24" style="margin-top: 15px">
          <el-row type="flex" justify="end">
            <el-button size="default" type="primary" @click="startAutoTask(currentAutoTask)">
              启动
            </el-button>
          </el-row>
        </el-col>
...
const onStartFlowEntryClick = (row: ANY_OBJECT) => {
  if (row.flowType === FlowEntryType.AUTO_TASK) {
    ...
    currentAutoTask.value = undefined;
    startFlowTask(row);
  }
};

const startAutoTask = (autoTask: ANY_OBJECT) => {
  let params = {
    processDefinitionKey: autoTask.processDefinitionKey,
    variableData: autoTask.autoParamJson.reduce((retObj, item) => {
...
const startFlowTask = (row: ANY_OBJECT) => {
  let params = {

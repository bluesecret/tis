...
const isDraft = computed(() => {
  return typeof dialogParams.value.isDraft === 'string' ? dialogParams.value.isDraft === 'true' : dialogParams.value.isDraft;
});
const isPreview = computed(() => {
  return typeof dialogParams.value.isPreview === 'string' ? dialogParams.value.isPreview === 'true' : dialogParams.value.isPreview;
});
const isRuntime = computed(() => {
  return typeof dialogParams.value.isRuntime === 'string' ? dialogParams.value.isRuntime === 'true' : dialogParams.value.isRuntime;
});
const readOnly = computed(() => {
  return typeof dialogParams.value.readOnly === 'string' ? dialogParams.value.readOnly === 'true' : dialogParams.value.readOnly;
});
...
const variableList = computed(() => {
  ...
  const variableList = Array.isArray(temp)
    ? temp
    : JSON.parse(temp || '[]');
  return variableList;
});
    ...
    if (isOnlineForm.value && workflowFormRef.value.getFormData) {
      workflowFormRef.value
        ...
        .then((formData: ANY_OBJECT | null) => {
          ...
          const assigneeArr = assignee && assignee !== '' ? (assignee as string).split(',') : undefined;
...
const preHandlerOperationThen = (
  ...
  // 自由跳
  if (operation.type === SysFlowTaskOperationType.FREE_JUMP) {
    FlowOperationController.freeJump({
      processInstanceId: dialogParams.value.processInstanceId,
      sourceTaskId: dialogParams.value.taskId,

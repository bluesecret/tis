    ...
    >
      ...
      <el-form-item label="绑定值" prop="srcValue">
        ...
        <el-cascader
          ...
          :options="dialogParams.flowVariableList"
...
const onSubmit = () => {
  ...
  form.value.validate(valid => {
    if (valid) {
      ...
      if (formData.value.type === AutoTaskValueType.VARIABLE) {
        srcValue = Array.isArray(formData.value.srcValue)
          ? formData.value.srcValue[formData.value.srcValue.length - 1]
          : undefined;
      }
      let tempData = {
        ...formData.value,
...
onMounted(() => {
  if (dialogParams.value.data) {
    ...
    let tempSrcValue: Array<string | number> | undefined;
    if (dialogParams.value.data.type === AutoTaskValueType.VARIABLE) {
      tempSrcValue = findTreeNodePath(dialogParams.value.flowVariableList, srcValue);
    } else {
      tempSrcValue = undefined;
    }
    formData.value = {
      ...formData.value,
      ...dialogParams.value.data,
      srcValue: tempSrcValue || srcValue,
    };
  }
});
</script>

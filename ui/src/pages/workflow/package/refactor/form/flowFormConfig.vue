<template>
  <div>
    ...
    >
      ...
      <el-form-item
        label="移动端表单路由"
        prop="mobileRouterName"
        v-if="flowEntryComputed.bindFormType === SysFlowEntryBindFormType.ROUTER_FORM"
      >
        <el-input v-model="formData.mobileRouterName" clearable @change="updateElementFormKey" />
      </el-form-item>
      <el-form-item
        label="在线表单"
        prop="formId"
...
const resetFormList = () => {
  ...
  if (formObj) {
    formData.value = {
      ...
      mobileRouterName: formObj.mobileRouterName,
  ...
  form.value?.validate((valid: boolean) => {
    ...
    let formKeyString = JSON.stringify({
      ...
      mobileRouterName:
        flowEntry().value.bindFormType === SysFlowEntryBindFormType.ONLINE_FORM
          ? undefined
          : formData.value.mobileRouterName,
      formAuth: formData.value.formAuth,

<template>
  <div class="my-process-designer">
    ...
    <ProcessDingflowDesigner
      ref="dingflowDesigner"
      v-if="loaded && diagramType == DiagramType.DINGDING"
      :bpmnModeler="bpmnModeler"
      :downloadProcessAsXml="downloadProcessAsXml"
      :previewProcessXML="previewProcessXML"
      :openFile="openFile"
    />
    <el-dialog
      title="预览"
...
import ProcessDingflowDesigner from '@/pages/workflow/components/ProcessDingflowDesigner/index.vue';
// 代码高亮
...
const changeDiagram = () => {
  ElMessageBox.confirm('确认切换？切换后流程图数据重置！', '警告', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  })
    .then(() => {
      diagramType.value =
        diagramType.value === DiagramType.ORDINARY ? DiagramType.DINGDING : DiagramType.ORDINARY;
      loaded.value = false;
      nextTick(() => {
        createNewDiagram(null).then(() => {
          loaded.value = true;
          if (bpmnModeler) {
            const UserTask = bpmnModeler
              .get('elementRegistry')
              .find((el: ANY_OBJECT) => el.type === 'bpmn:UserTask');
            UserTask && bpmnModeler.get('selection').select(UserTask);
          }
        });
      });
    })

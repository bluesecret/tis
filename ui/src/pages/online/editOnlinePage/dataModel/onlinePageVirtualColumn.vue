...
const aggregationTable = computed(() => {
  if (currentColumn.value.aggregationTableId == null) return null;
  return findItemFromList(
    aggregationTableList.value,
    currentColumn.value.aggregationTableId,
    'tableId',
  );
});
...
const onAggregationTableIdChange = () => {
  ...
  if (aggregationTable.value != null) {
    if (
      Array.isArray(aggregationTable.value.columnList) &&
      aggregationTable.value.columnList.length > 0
    ) {
      aggregationColumnList.value = aggregationTable.value.columnList;
    } else {
      loadOnlineTableColumnList(aggregationTable.value.tableId).catch(e => {
        console.warn(e);
      });
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    OnlineColumnController.list(params)
      .then(res => {
        ...
        aggregationTable.value.columnList = res.data.dataList;
...
const buildVirtualColumnFilter = () => {
  ...
  filterList = filterList.map((item: ANY_OBJECT) => {
    return {
      ...item,
      table: aggregationTable.value,
      column: findItemFromList(aggregationTable.value?.columnList, item.columnId, 'columnId'),
    };
  });
  // 获取过滤条件种过滤值的字典信息
    ...
    if (newValue == null) {
      ...
      if (aggregationTable.value) {
        ...
        if (
          ...
          loadOnlineTableColumnList(aggregationTable.value.tableId)
            .then(res => {
              buildVirtualColumnFilter();
            })
            .catch(e => {

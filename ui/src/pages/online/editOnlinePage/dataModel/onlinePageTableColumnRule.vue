    ...
    <div class="column-attributes">
      ...
      >
        <div v-if="currentColumn != null" style="padding: 20px" class="attibute-box">
          ...
          <el-col
            class="attribute-item"
            v-if="currentColumn.fieldKind === SysOnlineFieldKind.UPLOAD"
          >
            <el-form-item label="文件类型">
              <el-select
                v-model="currentColumn.fileType"
                clearable
                placeholder="上传文件的类型（允许多选）"
                :multiple="true"
                :collapse-tags="true"
                @change="dirty = true"
                style="width: 278px"
              >
                <el-option
                  v-for="item in EnumFileType.getList()"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                />
          ...
          <el-col
            class="attribute-item"
            v-if="currentColumn.fieldKind === SysOnlineFieldKind.AUTO_CODE"
          >
            <el-form-item label="自动编码规则">
              <el-input
                v-model="currentColumn.encodedRule.prefix"
                style="width: 80px; margin-right: 5px"
                placeholder="前缀"
                @change="dirty = true"
              />
              <el-select
                v-model="currentColumn.encodedRule.precisionTo"
                clearable
                placeholder=""
                @change="dirty = true"
              ...
              <el-input
                v-model="currentColumn.encodedRule.middle"
                style="width: 80px; margin-right: 5px"
                placeholder="后缀"
                @change="dirty = true"
              />
              <el-input-number
                v-model="currentColumn.encodedRule.idWidth"
                style="width: 100px"
                placeholder="序号宽度"
                :controls="false"
                :min="1"
          ...
          <el-col
            class="attribute-item"
            v-if="currentColumn.fieldKind === SysOnlineFieldKind.FIELD_MASK"
          >
            <el-form-item label="字段脱敏规则">
              <el-select
                v-model="currentColumn.maskFieldType"
                placeholder="字段脱敏规则"
                @change="dirty = true"
                style="width: 278px"
              >
                <el-option
                  v-for="item in SysMaskFieldType.getList()"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                />
...
import { SysMaskFieldType } from '@/common/staticDict';
      ...
      res.data.dataList.forEach((item: ColumnInfo) => {
        let temp = {
          ...
          fileType: item.fileType
            ? item.fileType.split(',').map(item => Number.parseInt(item))
            : [],
          maxFileCount: item.maxFileCount,
          uploadFileSystemType: item.uploadFileSystemType || 0,
          filterType: item.filterType,
          dictId: item.dictId,
          maskFieldType: item.maskFieldType,
          encodedRule: item.encodedRule
            ? JSON.parse(item.encodedRule as string)
            : {
...
const saveColumn = () => {
  ...
  currentColumn.value.encodedRule.calculateWhenView =
    currentColumn.value.fieldKind === SysOnlineFieldKind.AUTO_CODE;
  if (currentColumn.value.encodedRule.calculateWhenView) {
    if (!currentColumn.value.encodedRule.prefix) {
      ElMessage.error('自动编码前缀不能为空！');
      return Promise.reject();
    }
    if (!currentColumn.value.encodedRule.precisionTo) {
      ElMessage.error('自动编码精度类型！');
      return Promise.reject();
    }
    if (
      currentColumn.value.encodedRule.idWidth == null ||
      currentColumn.value.encodedRule.idWidth <= 0
    ) {
      ElMessage.error('自动编码编号长度不能小于1！');
      return Promise.reject();
    }
  }
  return new Promise((resolve, reject) => {
    let params = {
      onlineColumnDto: {
        ...
        encodedRule: JSON.stringify(currentColumn.value.encodedRule),
        nullable: !currentColumn.value.required,
        maskFieldType: currentColumn.value.maskFieldType,
        fileType: Array.isArray(currentColumn.value.fileType)
          ? currentColumn.value.fileType.join(',')
          : currentColumn.value.fileType,
      },
    };
...
const setCurrentColumn = (column: ANY_OBJECT) => {
  if (column.maskFieldType == null) column.maskFieldType = SysMaskFieldType.CUSTOM;
...
const columnKindItemDisabled = (columnKind: ANY_OBJECT) => {
  switch (columnKind.id) {
    ...
    case SysOnlineFieldKind.AUTO_CODE:

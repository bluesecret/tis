    ...
    <div class="left-panel">
      ...
      <template v-else>
        ...
        >
          ...
          <el-row style="height: calc(100% - 65px)">
            <el-scrollbar style="width: 100%; height: 100%">
              <el-collapse ref="panelTableList" v-model="activeDataTable" :accordion="true">
                <el-collapse-item
        ...
        <div style="font-size: 18px">
          <i
            ...
            :class="{
              ...
              disabled: (currentForm || {}).formType === SysOnlineFormType.ADVANCE_QUERY,
            }"
            title="手机端"
      ...
      <div class="design-box" :key="currentFormId">
        <div style="max-width: 100%" :style="getDesignBoxStyle" @click="onFormClick">
          ...
          <OnlineMobileQueryForm
            v-if="
              (currentForm || {}).formType === SysOnlineFormType.QUERY && activeMode === 'mobile'
            "
            height="100%"
            :isEdit="true"
            :mode="activeMode"
          ...
          <OnlineMobileWorkOrderForm
            v-if="
              (currentForm || {}).formType === SysOnlineFormType.WORK_ORDER &&
              activeMode === 'mobile'
            "
            height="100%"
            :isEdit="true"
          ...
          <el-scrollbar :style="{ height: getClientHeight - 138 + 'px' }">
            <el-row style="width: 100%; padding: 16px">
              <CustomEventSetting
                ...
                v-model:value="currentWidget.eventList"
                :allowEventList="widgetAllowEventList"
                :formConfig="buildFormConfig"
                :mode="activeMode"
              />
              <CustomEventSetting
                v-else-if="currentForm != null"
                v-model:value="currentForm[activeMode].formEventList"
...
const formValidWidgetGroup = computed(() => {
  ...
  let tempList = (tempGroupList || []).filter((group: ANY_OBJECT) => {
    ...
    if (
      ...
      return group.id === 'filter' || activeMode.value === 'mobile';
    } else {
...
const onEditTableColumn = (table: ANY_OBJECT, column: ColumnInfo) => {
  Dialog.show(
    ...
    .then(res => {
      ...
      if (panelTableList.value) {
        nextTick(() => {
          panelTableList.value.$forceUpdate();
        });
      }
    })
    .catch(e => {
...
const loadVirtualColumnList = () => {
  let currentDatasource = getAllTableList.value.find(table => {
    if (table.relationType == null) {
      return table;
    }
  });
  ...
  OnlineVirtualColumnController.list(params)
    .then(res => {
      currentDatasource.columnList = currentDatasource.columnList.filter(
        item => !item.aggregationColumn,
      );
      currentDatasource.columnList.push(
        ...res.data.dataList.map(item => {
          return {
            ...
            parentKey: false,
            primaryKey: false,
            tableId: currentDatasource.tableId,
            tag: item,
          };
        }),
      );

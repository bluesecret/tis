...
const buildFormConfig = computed(() => {
  return {
    ...
    currentWidget: {
      widgetType: (formConfig().currentWidget || {}).widgetType,
      widgetId: (formConfig().currentWidget || {}).widgetId,
      widgetName: (formConfig().currentWidget || {}).widgetName,
    },
    getMasterTable: {
      ...
      columnList: formConfig().getMasterTable.columnList.map((column: ANY_OBJECT) => {
        return {
          ...
          objectFieldType: column.objectFieldType,
        };
      }),
    },
...
const onEditOperate = (row: ANY_OBJECT | null) => {
  Dialog.show<ANY_OBJECT>(
    ...
    EditCustomFormOperate,
    {
      area:
        row &&
        (row.type === SysCustomWidgetOperationType.EXPORT ||
          row.type === SysCustomWidgetOperationType.IMPORT)
          ? '900px'
          : '600px',
      offset: '50px',
    },

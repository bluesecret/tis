<template>
  <div class="form-single-fragment third-party-dlg" style="position: relative">
    ...
    >
      ...
      <el-form-item v-if="formData.fieldKind === SysOnlineFieldKind.AUTO_CODE" label="自动编码规则">
        <el-input
          v-model="formData.encodedRule.prefix"
          style="width: 80px; margin-right: 5px"
          placeholder="前缀"
          @change="dirty = true"
        />
        <el-select
          v-model="formData.encodedRule.precisionTo"
          clearable
          placeholder=""
          @change="dirty = true"
        ...
        <el-input
          v-model="formData.encodedRule.middle"
          style="width: 80px; margin-right: 5px"
          placeholder="后缀"
          @change="dirty = true"
        />
        <el-input-number
          v-model="formData.encodedRule.idWidth"
          style="width: 100px"
          placeholder="序号宽度"
          :controls="false"
          :min="1"
...
import { SysAutoCodeType } from '@/common/staticDict/flow';
...
const onSubmit = () => {
  formData.value.encodedRule.calculateWhenView =
    formData.value.fieldKind === SysOnlineFieldKind.AUTO_CODE;
  if (formData.value.encodedRule.calculateWhenView) {
    if (formData.value.encodedRule.prefix == null || formData.value.encodedRule.prefix === '') {
      ElMessage.error('自动编码前缀不能为空！');
      return Promise.reject();
    }
    if (
      formData.value.encodedRule.precisionTo == null ||
      formData.value.encodedRule.precisionTo === ''
    ) {
      ElMessage.error('自动编码精度类型！');
      return Promise.reject();
    }
    if (formData.value.encodedRule.idWidth == null || formData.value.encodedRule.idWidth <= 0) {
  ...
  let params = {
    onlineColumnDto: {
      ...
      encodedRule: JSON.stringify(formData.value.encodedRule),
  ...
  OnlineColumnController.update(params)
    .then(() => {
      ...
      if (props.dialog) {
        props.dialog.submit(params.onlineColumnDto);
      } else {
...
onMounted(() => {
  formData.value = {
    ...
    encodedRule: dialogParams.value.rowData.encodedRule
      ? JSON.parse(dialogParams.value.rowData.encodedRule)
      : {},
  };
});
</script>

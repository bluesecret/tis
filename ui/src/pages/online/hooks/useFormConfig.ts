...
export const useFormConfig = () => {
  const baseQueryForm = {
    ...
    operationList: [
      ...
      {
        id: 10,
        type: SysCustomWidgetOperationType.IMPORT,
        name: SysCustomWidgetOperationType.getValue(SysCustomWidgetOperationType.IMPORT),
        enabled: false,
        builtin: true,
        rowOperation: false,
        btnType: 'primary',
      ...
      {
        id: 20,
        type: SysCustomWidgetOperationType.START_FLOW,
        name: SysCustomWidgetOperationType.getValue(SysCustomWidgetOperationType.START_FLOW),
        enabled: false,
        builtin: true,
        rowOperation: true,
        btnType: 'primary',
  ...
  const baseMobileQueryForm = {
    ...
    labelPosition: 'right',
    tableWidget: {
      ...widgetData.getWidgetObject(queryListConfig),
    },
    leftWidget: {
      ...widgetData.getWidgetObject(treeConfig),
    },
  ...
  const baseMobileWorkOrderForm = {
    ...
    labelPosition: 'right',
    tableWidget: {
      ...widgetData.getWidgetObject(workOrderListConfig),
    },
    operationList: [],
    customFieldList: [],
    widgetList: [],
  ...
  const buildFormConfig = (formData: ANY_OBJECT) => {
    ...
    // 表格配置
    if (rawData && rawData.onlineFormUserExt) {
      formConfig.customConfig = {
        id: rawData.onlineFormUserExt.id,
        onlineFormId: rawData.onlineFormUserExt.onlineFormId,
        extraData: rawData.onlineFormUserExt.extraData
          ? JSON.parse(rawData.onlineFormUserExt.extraData)
          : {},
      };
    }
    if (formConfig.customConfig == null) formConfig.customConfig = {};

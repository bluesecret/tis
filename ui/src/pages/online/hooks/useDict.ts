...
export const useDict = () => {
  function getTableDictData(
    dictId: string,
    dictParams: ANY_OBJECT,
    rootDataWhenNoParentId = false,
  ) {
    return new Promise<ANY_OBJECT[]>((resolve, reject) => {
      ...
      const params = {
        ...
        rootDataWhenNoParentId: rootDataWhenNoParentId,
      };
  ...
  function getDictDataList(
    ...
    switch (dictInfo.dictType) {
      ...
      case SysOnlineDictType.CODE:
        return getTableDictData(dictInfo.dictId, dictParams, rootDataWhenNoParentId);

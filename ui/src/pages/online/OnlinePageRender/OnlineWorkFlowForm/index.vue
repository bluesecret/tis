      ...
      .then(res => {
        ...
        Object.keys(masterData).forEach(key => {
          ...
          if (relationVariableName == null) {
            ...
            // 从表字段
            if (relation.relationType === SysOnlineRelationType.ONE_TO_MANY && masterData[key]) {
              // 如果是一对多关联字段，判断表单上使用这个字段的table组件是否支持数据加载数据后脚本
              if (Array.isArray(tableWidgetList)) {
                tableWidgetList.forEach(tableWidget => {
                  if (
                    tableWidget.relation &&
                    tableWidget.relation.relationId === relation.relationId
                  ) {
                    if (
                      typeof tableWidget.eventInfo[OnlineFormEventType.AFTER_LOAD_TABLE_DATA] ===
                      'function'
                    ) {
                      let params: ANY_OBJECT = { formData };
                      masterData[key] = tableWidget.eventInfo[
                        OnlineFormEventType.AFTER_LOAD_TABLE_DATA
                      ](masterData[key], useFormExpose(formData, props));
                    }
                  }
                });
              }
...
const viewTaskFormKey = () => {
  if (
    props.flowInfo == null ||
    props.flowInfo.processDefinitionKey == null ||
    props.flowInfo.processDefinitionKey === ''
  ) {
      ...
      initFormData()
        .then(() => {
          if (
            ...
            form2.value.eventInfo[OnlineFormEventType.AFTER_LOAD_FORM_DATA](
              useFormExpose(formData, props),
            );
          }
          initWidgetLinkage();
          setTimeout(() => {
            if (componentRef.value) componentRef.value.clearValidate();
          });
        })
        .catch((e: Error) => {
          console.warn(e);
...
const getFormDataImpl = (variableList: ANY_OBJECT[] | null = null) => {
  ...
  if (variableList && variableList.value) {
    variableList.value.forEach(variable => {
      if (!variable.builtin) {
        let column = form2.value.columnMap.get(variable.bindColumnId);
        let relation = form2.value.relationMap.get(variable.bindRelationId);
        if (column != null) {
  ...
  if (
    ...
    tempObj = form2.value.eventInfo[OnlineFormEventType.BEFORE_COMMIT_FORM_DATA](
      tempObj,
      useFormExpose(formData, props),
    );
  }

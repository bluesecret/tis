        ...
        <el-main style="width: 100%; padding: 25px; background: #f9f9f9">
          <el-row
            ...
            :style="{
              height:
                'calc(100vh - ' +
                (dialogParams.readOnly && (form2.operationList || []).length <= 0 ? 122 : 192) +
                'px)',
            }"
          >
            <div
...
const onSaveFormData = () => {
  ...
  if (
    ...
    params = form2.value.eventInfo[OnlineFormEventType.BEFORE_COMMIT_FORM_DATA](
      params,
  ...
  formRef.value.validate((valid: boolean) => {
    ...
    if (dialogParams.value.saveData) {
      ...
      if (
        ...
        form2.value.eventInfo[OnlineFormEventType.BEFORE_COMMIT_FORM_DATA](
          null,
          useFormExpose(formData, props),
        );
      }
...
const loadRelationTableData = (widget: ANY_OBJECT) => {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    if (
      ...
      params = widget.eventInfo[OnlineFormEventType.BEFORE_LOAD_TABLE_DATA](
        params,
        useFormExpose(formData, props),
      );
    }
    ...
    httpCall
      .then(res => {
        if (
          ...
          res.data.dataList = widget.eventInfo[OnlineFormEventType.AFTER_LOAD_TABLE_DATA](
            res.data.dataList,
            useFormExpose(formData, props),
          );
        }
...
const formExpose = useFormExpose(formData, props);
...
onMounted(() => {
  ...
  if (!dialogParams.value.isEdit) {
    if (
      ...
      form2.value.eventInfo[OnlineFormEventType.AFTER_CREATE_FORM](formExpose);
    }
    initWidgetRule();
    initFormData()
      .then(() => {
        if (
          ...
          form2.value.eventInfo[OnlineFormEventType.AFTER_LOAD_FORM_DATA](formExpose);
        }
        initWidgetLinkage();
        setTimeout(() => {

  ...
  >
    <el-row :gutter="20" class="full-width-input">
      ...
      <el-col
        :span="12"
        v-if="
          formData.menuType === SysMenuType.MENU && formData.bindType === SysMenuBindType.REPORT
        "
      >
...
const reportPagePath = ref<string[]>([]);
...
const onReportPageChange = (values: CascaderValue) => {
  formData.value.reportPageId = Array.isArray(values)
...
const loadReportPageList = () => {
  ReportPageGroupController.listAll({})
    .then(res => {
      ...
      if (formData.value.reportPageId) {
  ...
  setTimeout(() => {
    if (props.rowData != null) {
      ...
      if (extraData != null) {
        ...
        } else if (formData.value.reportPageId != null) {
          // 绑定报表
          formData.value.bindType = SysMenuBindType.REPORT;

  ...
  <div class="form-single-fragment" style="position: relative">
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="24" v-if="formData.SysDataPerm.ruleType === SysDataPermType.SQL">
          <el-form-item label="自定义SQL" prop="SysDataPerm.customSql">
            <el-input
              type="textarea"
              style="height: 50px"
              placeholder="不能使用逻辑操作符 AND/OR 开头，SQL 中可使用变量 loginUserId、loginDeptId、loginName、loginPostIds、loginDeptPostIds、loginRoleIds，变量使用方式为 ${loginUserId}"
              v-model="formData.SysDataPerm.customSql"
              clearable
            />
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-tabs class="dataperm-data" type="border-card">
              ...
              <el-row>
                ...
                <el-col :span="24">
                  <el-scrollbar
                    :style="{
                      height:
                        formData.SysDataPerm.ruleType === SysDataPermType.SQL ? '215px' : '283px',
                    }"
            ...
            <el-tab-pane label="菜单列表">
              <el-scrollbar
                :style="{
                  height: formData.SysDataPerm.ruleType === SysDataPermType.SQL ? '262px' : '330px',
                }"
                wrap-class="scrollbar_dropdown__wrap"
              >
                <div
...
import { SysDataPermController, MobileEntryController, DictionaryController } from '@/api/system';
...
const sudokuTreeNode = ref();
const bannerTreeNode = ref();
const deptTree = ref<DictData[]>([]);
const sudokuTree = ref<ANY_OBJECT[]>([]);
const bannerTree = ref<ANY_OBJECT[]>([]);
...
const rules = {
  ...
  'SysDataPerm.customSql': [{ required: true, message: '请输入自定义SQL', trigger: 'blur' }],
};
const onCancel = () => {
  if (props.dialog) {
    props.dialog.cancel();
...
const onUpdateClick = () => {
  form.value.validate((valid: boolean) => {
    ...
    let params = {
      sysDataPermDto: {
        ...
        extraData: JSON.stringify({
          customSql: formData.SysDataPerm.customSql,
        }),
      },
    } as ANY_OBJECT;
    if (
    ...
    // 九宫格、轮播图选项
    let mobileEntryList = sudokuTreeNode.value.getCheckedKeys();
    mobileEntryList = mobileEntryList.concat(bannerTreeNode.value.getCheckedKeys());
    params.entryIdListString = Array.isArray(mobileEntryList)
      ? mobileEntryList.join(',')
      : undefined;
    ...
    if (props.dataPermId == null) {
      ...
      SysDataPermController.view(params)
        .then(res => {
          ...
          formData.SysDataPerm = { ...res.data, customSql: extraData.customSql };
          resolve(res);
        })
        .catch(e => {
          console.warn(e);

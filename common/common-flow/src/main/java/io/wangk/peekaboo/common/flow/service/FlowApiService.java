...
import io.wangk.peekaboo.common.flow.object.AnalyzedNode;
...
public interface FlowApiService {
     ...
     * @return 新启动的流程实例。
     */
    ProcessInstance start(String processDefinitionId, Object dataId, JSONObject taskVariableData);
     ...
     * @param taskKeys          复活后的任务标识集合。
     * @param taskComment       流程复活备注。
     * @return 新复活的流程实例。
     */
    ProcessInstance revive(String processInstanceId, List<String> taskKeys, String taskComment);
     ...
     * @param option           操作选项。
     */
    void completeTask(Task task, FlowTaskComment flowTaskComment, JSONObject taskVariableData, FlowApiOption option);
     ...
     * @param fieldExtensions 执行监听器的扩展变量列表。
     */
    void addExecutionStartListener(
            FlowElement flowElement,
            Class<? extends ExecutionListener> listenerClazz,
            List<FieldExtension> fieldExtensions);
     ...
     *
     * @param processDefinitionId 流程定义Id。
     * @return 流程图中所有自动化接收任务的映射。
     */
    Map<String, ReceiveTask> getAllAutoTaskMap(String processDefinitionId);
     ...
     * @return 流程元素。
     */
    <T extends FlowElement> T getFlowElement(String processDefinitionId, String taskKey, Class<T> clazz);
     ...
     * @return 流程元素。
     */
    <T extends FlowElement> T getFlowElement(BpmnModel bpmnModel, String taskKey, Class<T> clazz);

    /**
     * 判断字符串是否为变量。
     *
     * @param s 字符串。
     * @return 是变量返回true，否则false。
     */
    boolean isVariableString(String s);

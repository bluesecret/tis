...
import io.wangk.peekaboo.common.flow.object.FlowTaskCommentExtra;
import lombok.Data;
...
public class FlowTaskComment {
    ...
    public void enableRejectBackType(String sourceTaskId, String targetTaskKey, String targetUser) {
        FlowTaskCommentExtra extra;
        if (customBusinessData == null) {
            extra = new FlowTaskCommentExtra();
        } else {
            extra = JSON.parseObject(customBusinessData, FlowTaskCommentExtra.class);
        }
        extra.setRejectBackType(true);
        extra.setRejectBackSourceTaskId(sourceTaskId);
        extra.setRejectBackTargetTaskKey(targetTaskKey);
        extra.setRejectBackTargetUser(targetUser);
        this.customBusinessData = JSON.toJSONString(extra);
    }
}

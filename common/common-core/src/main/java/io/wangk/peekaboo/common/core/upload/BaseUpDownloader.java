...
public abstract class BaseUpDownloader {
    ...
    public String makeFullPath(
        ...
        if (StringUtils.isNotBlank(rootBaseDir)) {
            uploadPathBuilder.append(rootBaseDir).append(StrUtil.SLASH);
        }
        if (Boolean.TRUE.equals(asImage)) {
            uploadPathBuilder.append(ApplicationConstant.UPLOAD_IMAGE_PARENT_PATH).append(StrUtil.SLASH);
        } else {
            uploadPathBuilder.append(ApplicationConstant.UPLOAD_ATTACHMENT_PARENT_PATH).append(StrUtil.SLASH);
        }
        if (StringUtils.isNotBlank(modelName)) {
            uploadPathBuilder.append(modelName).append(StrUtil.SLASH);
        }
        if (StringUtils.isNotBlank(fieldName)) {
            uploadPathBuilder.append(fieldName).append(StrUtil.SLASH);
        }
        return StrUtil.removeSuffix(uploadPathBuilder.toString(), StrUtil.SLASH);
    }

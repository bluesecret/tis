...
import io.wangk.peekaboo.common.core.annotation.RelationGlobalDict;
...
public class ImportUtil {
    ...
    private static Object transDictData(boolean multiSelect, Object value, Map<String, Object> inversedDictMap) {
        if (!multiSelect) {
            return inversedDictMap.get(value.toString());
        }
        List<String> newValues = StrUtil.split(value.toString(), StrUtil.COMMA);
        List<Object> keyList = newValues.stream()
                .map(inversedDictMap::get).filter(Objects::nonNull).collect(Collectors.toList());
        return CollUtil.join(keyList, StrUtil.COMMA);
    }
    ...
    private static class MyRowHandler implements RowHandler {
        private final ImportHeaderInfo[] headers;
        private final Map<Integer, ImportHeaderInfo> headerInfoMap;

package io.wangk.peekaboo.common.core.base.service;
...
public abstract class BaseService<M, K extends Serializable> extends ServiceImpl<BaseDaoMapper<M>, M> implements IBaseService<M, K> {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void updateBatch(M data, Set<K> idSet) {
        QueryWrapper<M> queryWrapper = new QueryWrapper<>();
        if (CollUtil.isNotEmpty(idSet)) {
            queryWrapper.in(idColumnName, idSet);
        }
        mapper().update(data, queryWrapper);
    }

    @Transactional(rollbackFor = Exception.class)
    @Override
    public boolean update(M entity, Wrapper<M> updateWrapper) {
    ...
    private void doBuildManyToManyAggregationForDataList(
        ...
        if (StrUtil.isNotBlank(relationStruct.service.deletedFlagFieldName)
                && !basicRelationInfo.onlySelectRelationTable) {
    ...
    private Tuple2<String, String> makeSelectListAndGroupByClause(
        ...
        if (aggregationType.equals(AggregationType.CONCAT)) {
            String aggrClause = this.makeConcatAggregationColumn(aggregationTableName, aggregationColumn, groupColumnName);
    ...
    private String makeConcatAggregationColumn(String tableName, String aggregationColumn, String groupColumn) {
        StringBuilder groupedSelectList = new StringBuilder(128);

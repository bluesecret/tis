package io.wangk.peekaboo.common.dbutil.util;
...
import io.wangk.peekaboo.common.core.exception.InvalidDataFieldException;
import io.wangk.peekaboo.common.core.exception.InvalidDblinkTypeException;
...
import io.wangk.peekaboo.common.dbutil.provider.*;
import io.wangk.peekaboo.common.dbutil.constant.CustomDateValueType;
...
public abstract class DataSourceUtil {
    ...
    static {
        ...
        PROVIDER_MAP.put(DblinkType.SQLSERVER, new SqlServerProvider());
        PROVIDER_MAP.put(DblinkType.DAMENG, new DamengProvider());
        PROVIDER_MAP.put(DblinkType.KINGBASE, new KingbaseProvider());
        PROVIDER_MAP.put(DblinkType.OPENGAUSS, new OpenGaussSqlProvider());
        PROVIDER_MAP.put(DblinkType.CLICKHOUSE, new ClickHouseProvider());
        PROVIDER_MAP.put(DblinkType.DORIS, new DorisProvider());
    }
     ...
     * @return 数据库链接类型。
     */
    public abstract int getDblinkTypeByDblinkId(Long dblinkId);
     ...
     * @return 数据库链接配置信息。
     */
    public abstract String getDblinkConfigurationByDblinkId(Long dblinkId);
    ...
    public static void testConnection(
        ...
        switch (databaseType) {
            ...
            case DblinkType.OPENGAUSS:
                ...
                urlBuilder.append("&TimeZone=Asia/Shanghai&useUnicode=true&characterEncoding=utf8");
                break;
            case DblinkType.CLICKHOUSE:
                urlBuilder.append("jdbc:clickhouse://")
                        .append(hostAndPort)
                        .append("/")
                        .append(databaseName);
                break;
     ...
     * @param tableQuery true为表查询，否则基于SQL的结果集查询。
     * @return 可执行的SQL语句。
     */
    public String getQuerySql(Long dblinkId, String tableOrSql, DatasetParam param, boolean tableQuery) {
        String sql;
        if (tableQuery) {
            sql = "SELECT * FROM " + tableOrSql;
            if (CollUtil.isNotEmpty(param.getSelectColumnNameList())) {
                sql = SQL_SELECT + StrUtil.join(",", param.getSelectColumnNameList()) + " FROM " + tableOrSql;
            }
        } else {
            sql = SQL_SELECT_FROM + tableOrSql + SQL_AS_TMP;
            if (CollUtil.isNotEmpty(param.getSelectColumnNameList())) {
                sql = SQL_SELECT + StrUtil.join(",", param.getSelectColumnNameList()) + " FROM (" + tableOrSql + SQL_AS_TMP;
            }
        }
        Tuple2<String, List<Object>> filterTuple = this.buildWhereClauseByFilters(dblinkId, param.getFilter());
        sql += filterTuple.getFirst();
        List<Object> paramList = filterTuple.getSecond();
        ...
        if (param.getOrderBy() != null) {
            sql += SQL_ORDER_BY + param.getOrderBy();
        }
        return sql;
    }
    ...
    public List<SqlTableColumn> getSqlColumnList(Long dblinkId, Connection conn, String sql) {
        String querySql = "SELECT * FROM (" + sql + ") orange_tmp where 1 = 0";
        try {
    ...
    public SqlResultSet<Map<String, Object>> getSqlDataList(
        ...
        String fullSql = SQL_SELECT_FROM + sql + SQL_AS_TMP;
        if (CollUtil.isNotEmpty(datasetParam.getSelectColumnNameList())) {
            fullSql = SQL_SELECT +
                    StrUtil.join(",", datasetParam.getSelectColumnNameList()) + " FROM (" + sql + SQL_AS_TMP;
        }
        Tuple2<String, List<Object>> filterTuple = this.buildWhereClauseByFilters(dblinkId, datasetParam.getFilter());
    ...
    private String makeSqlParamLogString(List<Object> paramList) {
        ...
        return parameterValues.length() == 0
                ? parameterValues.toString() : parameterValues.substring(0, parameterValues.length() - 2);
    }

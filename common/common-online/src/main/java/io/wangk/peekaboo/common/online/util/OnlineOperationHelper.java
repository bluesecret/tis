...
import io.wangk.peekaboo.common.online.object.BaseOnlineExtendExecutor;
import io.wangk.peekaboo.common.online.object.ColumnData;
import io.wangk.peekaboo.common.online.object.OnlinePageExtra;
...
public class OnlineOperationHelper {
     ...
     *
     * @param datasourceId 数据源Id。
     * @param variableName 待验证的数据源变量名。
     * @return 数据源详情数据。
     */
    public ResponseResult<OnlineDatasource> verifyAndGetDatasource(Long datasourceId, String variableName) {
        ResponseResult<OnlineDatasource> verifyResult = this.verifyAndGetDatasource(datasourceId);
        if (!verifyResult.isSuccess()) {
            return verifyResult;
        }
        if (!verifyResult.getData().getVariableName().equals(variableName)) {
        ...
        return ResponseResult.success(verifyResult.getData());
    }
    ...
    /**
     * 构建在线表的部分更新数据记录。仅仅更新存在数据的字段。
     *
     * @param table     在线数据表对象。
     * @param tableData 在线数据表数据。
     * @return 在线表的数据记录。
     */
    public ResponseResult<List<ColumnData>> buildTablePartialData(OnlineTable table, JSONObject tableData) {
        List<ColumnData> columnDataList = new LinkedList<>();
        for (OnlineColumn column : table.getColumnMap().values()) {
            Object value = this.getColumnValue(tableData, column);
            if (value != null) {
    ...
    /**
     * 将与指定数据源Id关联的OnlinePage对象中，配置的在线表单后台扩展执行器对象写入线程本地化存储。
     *
     * @param datasourceId 数据源Id。
     */

...
import io.wangk.peekaboo.common.dict.constant.GlobalDictItemStatus;
...
import io.wangk.peekaboo.common.online.util.OnlineOperationHelper;
import io.wangk.peekaboo.common.online.util.OnlineConstant;
...
import io.wangk.peekaboo.common.online.model.constant.RelationType;
...
public class OnlineOperationController {
    ...
    public ResponseResult<Void> updateDatasource(
            ...
            @MyRequestBody JSONObject slaveData) {
        ResponseResult<OnlineDatasource> datasourceResult =
                onlineOperationHelper.verifyAndGetDatasource(datasourceId, datasourceVariableName);
        if (!datasourceResult.isSuccess()) {
            return ResponseResult.errorFrom(datasourceResult);
        }
        OnlineDatasource datasource = datasourceResult.getData();
     ...
     * @param datasourceId           主表数据源Id。
     * @param masterData             表数据。这里没有包含的字段将视为NULL。
     * @param dataIdList             主键Id集合，如果为空就全部更新。
     * @return 应该结果。
     */
    @SaTokenDenyAuth
    @OperationLog(type = SysOperationLogType.UPDATE)
    @PostMapping("/updateDatasourceBatch/{datasourceVariableName}")
    public ResponseResult<Void> updateDatasourceBatch(
            @PathVariable("datasourceVariableName") String datasourceVariableName,
            @MyRequestBody(required = true) Long datasourceId,
            @MyRequestBody(required = true) JSONObject masterData,
            @MyRequestBody List<String> dataIdList) {
        ResponseResult<OnlineDatasource> datasourceResult =
                onlineOperationHelper.verifyAndGetDatasource(datasourceId, datasourceVariableName);
        if (!datasourceResult.isSuccess()) {
            return ResponseResult.errorFrom(datasourceResult);
        ...
        OnlineTable masterTable = datasource.getMasterTable();
        onlineOperationHelper.enableOnlineExtendExecutor(datasourceId);
        onlineOperationService.updateBatch(masterTable, masterData, dataIdList);
        return ResponseResult.success();
    }
    ...
    @SaTokenDenyAuth
    @OperationLog(type = SysOperationLogType.UPDATE)
    @PostMapping("/updateOneToManyRelationBatch/{datasourceVariableName}")
    public ResponseResult<Void> updateOneToManyRelationBatch(
            @PathVariable("datasourceVariableName") String datasourceVariableName,
            @MyRequestBody(required = true) Long datasourceId,
            @MyRequestBody(required = true) Long relationId,
            @MyRequestBody(required = true) JSONObject slaveData,
        ...
        OnlineTable slaveTable = verifyResult.getData().getSlaveTable();
        onlineOperationHelper.enableOnlineExtendExecutor(datasourceId);
        onlineOperationService.updateBatch(slaveTable, slaveData, dataIdList);
        return ResponseResult.success();
    }
    ...
    public ResponseResult<String> print(
        ...
        MyPageData<Map<String, Object>> pageData = onlineOperationService.getMasterDataList(
                table, null, null, filters, null, null, null);
        if (pageData.getDataList().size() != idSet.size()) {
     ...
     * @param datasourceVariableName 数据源名称。
     * @param datasourceId           数据源Id。
     * @param importColumnIds        导入字段Id列表。
     * @param skipHeader             是否跳过头部。
    ...
    public ResponseResult<Void> importDatasource(
            @PathVariable("datasourceVariableName") String datasourceVariableName,
            @RequestParam Long datasourceId,
            @RequestParam List<Long> importColumnIds,
        ...
        this.doImport(datasourceResult.getData().getMasterTable(), importColumnIds, skipHeader, importFile);
        return ResponseResult.success();
    }
    ...
    public ResponseResult<Void> importOneToManyRelation(
            @PathVariable("datasourceVariableName") String datasourceVariableName,
            @RequestParam Long datasourceId,
            @RequestParam Long relationId,
            @RequestParam List<Long> importColumnIds,
        ...
        this.doImport(verifyResult.getData().getSlaveTable(), importColumnIds, skipHeader, importFile);
        return ResponseResult.success();
    }
     ...
     * @param customFilterGroupList  自定义高级查询分组列表。
    ...
    public ResponseResult<MyPageData<Map<String, Object>>> listByDatasourceId(
            ...
            @MyRequestBody List<OnlineFilterGroupDto> customFilterGroupList,
        ...
        ResponseResult<Void> filterDtoListResult = this.verifyFilterDtoList(filterDtoList, customFilterGroupList);
        ...
        onlineOperationHelper.enableOnlineExtendExecutor(datasourceId);
        MyPageData<Map<String, Object>> pageData = onlineOperationService.getMasterDataList(
                masterTable, oneToOneRelationList, allRelationList, filterDtoList, customFilterGroupList, orderBy, pageParam);
    ...
    public void exportByDatasourceId(
        ...
        onlineOperationHelper.enableOnlineExtendExecutor(datasourceId);
        MyPageData<Map<String, Object>> pageData = onlineOperationService.getMasterDataList(
                masterTable, oneToOneRelationList, allRelationList, filterDtoList, customFilterGroupList, orderBy, null);
        ...
        String filename = datasourceVariableName + "-" +
                MyDateUtil.toDateTimeString(DateTime.now(), MyDateUtil.COMMON_DATETIME_FILE_FORMAT) + ".xlsx";
    ...
    public ResponseResult<MyPageData<Map<String, Object>>> listByOneToManyRelationId(
        ...
        ResponseResult<Void> filterDtoListResult = this.verifyFilterDtoList(filterDtoList, null);
        if (!filterDtoListResult.isSuccess()) {
            return ResponseResult.errorFrom(filterDtoListResult);
        }
     ...
     * @param rootDataWhenNoParentId 对于树形字典，如果为true，当没有parentId过滤时返回parentId为null的顶层数据，否则返回全部。
     * @return 字典数据列表。
     */
    ...
    public ResponseResult<List<Map<String, Object>>> listDict(
            ...
            @MyRequestBody List<OnlineFilterDto> filterDtoList,
            @MyRequestBody Boolean rootDataWhenNoParentId) {
        String errorMessage;
        ...
        if (dict.getDictType().equals(DictType.GLOBAL_DICT)) {
            List<? extends GlobalDictItem> dictItems;
            if (tokenData.getTenantId() != null) {
                ...
                dictItems = tenantGlobalDictService.getGlobalDictItemListFromCache(tenantGlobalDict, null);
            } else {
                dictItems = globalDictService.getGlobalDictItemListFromCache(dict.getDictCode(), null);
            }
            dictItems = dictItems.stream()
                    .filter(item -> item.getStatus().equals(GlobalDictItemStatus.NORMAL))
        ...
        if (BooleanUtil.isTrue(rootDataWhenNoParentId)) {
            resultList = resultList.stream()
                    .filter(r -> ObjectUtil.isEmpty(r.get(ApplicationConstant.PARENT_ID)))
                    .collect(Collectors.toList());
        }
    ...
    private ResponseResult<Void> verifyFilterDtoList(
            List<OnlineFilterDto> filterDtoList, List<OnlineFilterGroupDto> filterGroupDtoList) {
        List<OnlineFilterDto> verifyFilterList = new ArrayList<>();
        if (CollUtil.isNotEmpty(filterDtoList)) {
            verifyFilterList.addAll(filterDtoList);
        }
        if (CollUtil.isNotEmpty(filterGroupDtoList)) {
            filterGroupDtoList.stream()
                    .filter(filterGroup -> filterGroup.getFilterList() != null)
                    .forEach(filterGroup -> verifyFilterList.addAll(filterGroup.getFilterList()));
        }
        String errorMessage;
        for (OnlineFilterDto filter : filterDtoList) {
    ...
    private void doImport(OnlineTable table, List<Long> importColumnIds, Boolean skipHeader, MultipartFile importFile)
            throws IOException {
        String filename = ImportUtil.saveImportFile(onlineProperties.getUploadFileBaseDir(), null, importFile);
        Map<Long, OnlineColumn> columnMap = table.getColumnMap();
        ImportUtil.ImportHeaderInfo[] headerInfos = new ImportUtil.ImportHeaderInfo[importColumnIds.size()];
        int index = 0;
        for (Long columnId : importColumnIds) {
            ImportUtil.ImportHeaderInfo headerInfo = new ImportUtil.ImportHeaderInfo();
            OnlineColumn onlineColumn = columnMap.get(columnId);
            headerInfo.setFieldName(onlineColumn.getColumnName());
            headerInfo.setFieldType(this.makeImportHeaderInfoByFieldType(onlineColumn.getObjectFieldType()));
            if (onlineColumn.getDictId() != null) {
                headerInfo.setFieldType(ImportUtil.STRING_TYPE);
            }
            headerInfo.setIndex(index);
            headerInfos[index++] = headerInfo;
        }
        List<Map<String, Object>> dataList = ImportUtil.doImport(headerInfos, skipHeader, filename);
        List<OnlineColumn> dictColumns = columnMap.values().stream()
                .filter(c -> c.getDictId() != null && importColumnIds.contains(c.getColumnId()))
                .collect(Collectors.toList());
        onlineOperationService.translateDictData(dataList, table, dictColumns);
        onlineOperationService.saveNewBatch(table, dataList.stream().map(JSONObject::new).collect(Collectors.toList()));
    }

    private int makeImportHeaderInfoByFieldType(String objectFieldType) {
        switch (objectFieldType) {
            case ObjectFieldType.INTEGER:
                return ImportUtil.INT_TYPE;
            case ObjectFieldType.LONG:
                return ImportUtil.LONG_TYPE;
            case ObjectFieldType.STRING:
                return ImportUtil.STRING_TYPE;
            case ObjectFieldType.BOOLEAN:
                return ImportUtil.BOOLEAN_TYPE;
            case ObjectFieldType.DATE:
                return ImportUtil.DATE_TYPE;
            case ObjectFieldType.DOUBLE:
                return ImportUtil.DOUBLE_TYPE;
            case ObjectFieldType.BIG_DECIMAL:
                return ImportUtil.BIG_DECIMAL_TYPE;
            default:

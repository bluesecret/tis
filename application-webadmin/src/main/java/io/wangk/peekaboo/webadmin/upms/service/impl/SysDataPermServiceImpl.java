...
import io.wangk.peekaboo.webadmin.upms.dao.SysDataPermDeptMapper;
import io.wangk.peekaboo.webadmin.upms.dao.SysDataPermMapper;
import io.wangk.peekaboo.webadmin.upms.dao.SysDataPermUserMapper;
import io.wangk.peekaboo.webadmin.upms.dao.SysDataPermMenuMapper;
...
public class SysDataPermServiceImpl extends BaseService<SysDataPerm, Long> implements SysDataPermService {
    ...
    private Map<Integer, String> mergeAndOptimizeDataPermRule(List<SysDataPerm> dataPermList, Long deptId) {
        ...
        List<SysDataPerm> customSqlDataPermList = ruleMap.get(DataPermRuleType.TYPE_CUSTOM_SQL);
        if (customSqlDataPermList != null) {
            List<String> extraDataList = customSqlDataPermList.stream()
                    .map(SysDataPerm::getExtraData)
                    .filter(StrUtil::isNotBlank)
                    .map(JSON::parseObject)
                    .map(extraData -> extraData.getString("customSql"))
                    .filter(StrUtil::isNotBlank)
                    .collect(Collectors.toList());
            resultMap.put(DataPermRuleType.TYPE_CUSTOM_SQL, JSON.toJSONString(extraDataList));
        }
        return resultMap;
    }

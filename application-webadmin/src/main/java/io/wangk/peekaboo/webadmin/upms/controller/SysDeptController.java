...
import io.wangk.peekaboo.webadmin.upms.vo.*;
import io.wangk.peekaboo.webadmin.upms.dto.*;
...
public class SysDeptController {
    ...
    public ResponseResult<Void> deleteSysDeptPost(@MyRequestBody Long deptId, @MyRequestBody Long postId) {
        ...
        SysDeptPost sysDeptPost = sysDeptService.getSysDeptPost(deptId, postId);
        if (sysDeptPost != null) {
            List<SysUser> deptPostUserList =
                    sysUserService.getSysUserListByDeptPostId(sysDeptPost.getDeptPostId(), null, null);
            if (!deptPostUserList.isEmpty()) {
                errorMessage = "数据验证失败，当前部门岗位存在用户，不能直接删除！";
                return ResponseResult.error(ErrorCodeEnum.DATA_VALIDATED_FAILED, errorMessage);
            }
        }
